...
```{r}
# clusters <- readRDS(paste0("only_",TRIAL,"_cart19_clusters.rds")) %>%
#   as.data.frame(row.names = NULL) %>%
#   filter(clus.origin=='Timepoint') %>% 
#   GenomicRanges::makeGRangesFromDataFrame(keep.extra.columns = TRUE)
# clusters %>% as.data.frame(row.names = NULL)
```
...
scan_sites <- gintools:::scan_format(
  tdn_sites, timepoint_sites, grouping = "specimen")
clust_name <- file.path(workingDir,paste0('ONLY_',TRIAL),paste0("only_",TRIAL,"_long_clusters.rds"))
