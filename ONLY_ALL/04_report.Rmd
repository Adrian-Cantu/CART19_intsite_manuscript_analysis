...
knit: (
  function(inputFile, encoding) { 
    ...
    rmarkdown::render( 
      ...
      output_file = paste(ptrial,'_goi_', Sys.Date(), '.pdf', sep='')) })
---
...
cond_uniq_sites <- readRDS(file.path(workingDir,paste0('ONLY_',TRIAL),paste0("only_",TRIAL,"_condensed_intsites.rds"))) #TODO
...
gene_stats <- read.csv( # TODO, not sure where this comes from
    file.path(workingDir,paste0('ONLY_',TRIAL),paste0("only_",TRIAL,"_cart19_gene_stats.csv"))) %>%
...
gene_impact <- readRDS( # TODO, not sure where this comes from
    file.path(workingDir,paste0('ONLY_',TRIAL),paste0("only_",TRIAL,"_cart19_gene_impact.rds"))) %>%
  dplyr::select(
...
saveRDS(df5,file = file.path('ALL_reports','df5.rds'))
saveRDS(df6,file = file.path('ALL_reports','df6.rds'))
saveRDS(gene_list_pct,file = file.path('ALL_reports','gene_list_pct.rds'))
saveRDS(df6.1,file = file.path('ALL_reports','df6.1.rds'))
saveRDS(go6,file = file.path('ALL_reports','go6.rds'))
saveRDS(kegg6,file = file.path('ALL_reports','kegg6.rds'))
saveRDS(df1.2,file = file.path('ALL_reports','df1.2.rds'))
saveRDS(df1.4,file = file.path('ALL_reports','df1.4.rds'))
saveRDS(df2.2,file = file.path('ALL_reports','df2.2.rds'))
saveRDS(df3.2,file = file.path('ALL_reports','df3.2.rds'))
saveRDS(onco_genes,file = file.path('ALL_reports','onco_genes.rds'))
...
openxlsx::saveWorkbook(wb,
  file.path(workingDir,'ONLY_ALL','goi_reports',paste0('goi_comprehensive_',TRIAL,'_',format(Sys.time(), "%Y%m%d"),'.xlsx')),
  overwrite = TRUE)
```
