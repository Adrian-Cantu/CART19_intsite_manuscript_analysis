...
cond_uniq_sites <- readRDS(file.path(workingDir,'ONLY_ALL',"only_ALL_condensed_intsites.rds")) #TODO
...
gene_stats <- read.csv( # TODO, not sure where this comes from
    file.path(workingDir,'ONLY_ALL',"only_ALL_cart19_gene_stats.csv")) %>%
...
df3.2 <- df3 %>%
  dplyr::arrange(
    desc(TP_TDN_max_span), desc(max_time), desc(TP_TDN_long_count), desc(TP_TDN_num_sites),
    desc(TP_TDN_num_patients), On_Onco_List
  ) %>%
  dplyr::select(
    gene_name, TP_TDN_max_span, max_time, TP_TDN_long_count, TP_TDN_num_patients, TP_TDN_num_sites,
