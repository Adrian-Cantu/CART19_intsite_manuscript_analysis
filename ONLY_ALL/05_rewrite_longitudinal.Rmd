...
df3.2 <- df3 %>%
  dplyr::arrange(
    desc(TP_TDN_top3avg),desc(TP_TDN_max_span), desc(max_time), desc(TP_TDN_long_count), desc(TP_TDN_num_sites),
  ...
  dplyr::select(
    ...
    TP_TDN_peak_abund,TP_TDN_peak_relAbund ,On_Onco_List,TP_TDN_top3avg
  ) %>%
  dplyr::mutate(TP_TDN_peak_relAbund=paste0(as.character(round(TP_TDN_peak_relAbund,digits = 2)),'%'),
      TP_TDN_top3avg=round(TP_TDN_top3avg,digits = 2) ) %>% 
  dplyr::rename(
    ...
    "Onco-Related" = On_Onco_List, "t3avg"=TP_TDN_top3avg
  )

```

Insertion sites that are close to Genes (less than 5,000bp) are grouped and labeled with that
gene name. Longitudinal gene of interest are those that have insertion sites in at least 10 different positions in at least 3 patients for both time 0 and later time samples. 
...
kable_styling(latex_options = c("repeat_header","striped"),font_size = 6)
