...
if(!exists('workingDir')) {workingDir <- "/home/ubuntu/data/CART19/CART19_from_git"}
if(!exists('scriptDir')) {scriptDir <- "/home/ubuntu/data/CART19/CART19_from_git/scripts"}
if(!exists('utilsDir')) {utilsDir <- "/home/ubuntu/data/CART19/CART19_from_git/utils"}
if(!exists('outputDir')) {outputDir <- "/home/ubuntu/data/CART19/CART19_from_git/data"}
if(!exists('numCores')) {numCores <- parallel::detectCores()}
if(!exists('opts_knit$get("output.dir")')){ opts_knit$set(output.dir = '../reports') }
options(bitmapType='cairo')
...
cond_uniq_sites <- readRDS(file.path(outputDir, "condensed_intsites.rds")) #TOCHANGE
...
cluster_list <- readRDS(file.path(outputDir, "cart19_clusters.rds")) # TOCHANGE, not sure where this comes from (scan stats)

gene_stats <- read.csv( # TOCHANGE, not sure where this comes from
...
gene_impact <- readRDS( # TOCHANGE, not sure where this comes from
...
```{r summary_tbl1, child = file.path("/home/ubuntu/data/CART19/CART19_from_git/scripts", "goi_child_templates", "summary_tbl1.rmd"), eval = nrow(df5) > 0,envir=environment()}
```
